...
public class FileTree extends JPanel {
  ...
  DefaultMutableTreeNode addNodes(DefaultMutableTreeNode curTop, File dir) {
    ...
    for (int i = 0; i < ol.size(); i++) {
      ...
      if (f.isDirectory())
        addNodes(curDir, f);
      else if(f.isFile() && f.getAbsolutePath().endsWith(".rb"))
    	  files.addElement(f);
    ...
    for (int fnum = 0; fnum < files.size(); fnum++){
    	...
    	File pfile = (File)files.elementAt(fnum);
    	DefaultMutableTreeNode fileNode = new DefaultMutableTreeNode(pfile.getName());
    	Page page = new Page();
		page.setPageFile(pfile);
		page.ProcessPage();
		POMutableTreeNode pageNode = new POMutableTreeNode();
  ...
  public static void main(String[] av) {
    ...
    if (av.length == 0) {
      cp.add(new FileTree(new File("C:\\azheng-QA-Workspace\\QA\\Cucumber\\Projects\\GreenDot")));
    } else {
